...
void a30_surface_to_portrait(SDL_Surface *surf, Uint32 *out) {
    ...
    if (g_display_rotation == 0) {
        /* Landscape direct copy (Miyoo Mini): row-by-row with alpha OR. */
#ifdef __ARM_NEON__
        const uint32x4_t alpha_v = vdupq_n_u32(0xFF000000u);
        for (int r = 0; r < g_panel_h; r++) {
            const Uint32 *in  = src + (size_t)r * (size_t)pitch;
            ...
            for (int c = 0; c < g_panel_w; c += 8) {
                vst1q_u32(row + c,     vorrq_u32(vld1q_u32(in + c),     alpha_v));
                vst1q_u32(row + c + 4, vorrq_u32(vld1q_u32(in + c + 4), alpha_v));
            }
        }
#else
        for (int r = 0; r < g_panel_h; r++) {
        ...
        for (int r = 0; r < g_panel_h; r++) {
            const int src_x   = g_display_w - 1 - r;
            Uint32   *row_out = out + (size_t)r * (size_t)g_panel_w;
            /* g_panel_w=480 divisible by 8 — no tail loop needed. */
            for (unsigned int c = 0; c < (unsigned)g_panel_w; c += 8) {
                __builtin_prefetch(&src[(c+8) * pitch + src_x], 0, 0);
                uint32_t tmp[8];
...
void a30_flip_video(const Uint32 *osd_portrait,
     ...
     *   display_w  — panel columns used by video (port_w at FIT, g_panel_w at FILL)
    ...
    int display_w = port_w + (int)((g_panel_w - port_w) * zoom_t + 0.5f);
    if (display_w > g_panel_w) display_w = g_panel_w;
    int col_off  = (g_panel_w - display_w) / 2;
    int src_rows = g_panel_h * port_w / display_w;
    if (src_rows > g_panel_h) src_rows = g_panel_h;
    int row_off  = (g_panel_h - src_rows) / 2;
    ...
    if (zoom_t < 0.001f) {
        if (!osd_portrait) {
            /* No OSD: pure sequential blit with black letterboxes. */
            for (int r = 0; r < g_panel_h; r++) {
                ...
                for (int c = col_off + port_w; c < g_panel_w; c++)
                    out[c] = 0xFF000000u;
            }
            return;
        }
        ...
        for (int r = 0; r < g_panel_h; r++) {
            Uint32       *out      = fb + (size_t)r * (size_t)s_fb_stride;
            const Uint32 *osd_row  = osd_portrait  + (size_t)r * (size_t)g_panel_w;
    ...
    /* Zoom path (WIDE/FILL): nearest-neighbour scale. */
    for (int r = 0; r < g_panel_h; r++) {
        int src_row = row_off + r * src_rows / g_panel_h;
        const Uint32 *port_row = portrait_bgra  + (size_t)src_row * (size_t)port_w;
        const Uint32 *osd_row  = osd_portrait ? osd_portrait + (size_t)r * (size_t)g_panel_w : NULL;
        Uint32       *out      = fb + (size_t)r * (size_t)s_fb_stride;
