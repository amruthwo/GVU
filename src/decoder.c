...
int decoder_probe(const char *path, ProbeInfo *info,
    ...
    /* Cap probe I/O — default 5 MB / 5 s is far too slow on SD card.
       MP4/MKV store codec info in the container header so limits are not hit. */
    ...
    int best_video = av_find_best_stream(fmt, AVMEDIA_TYPE_VIDEO, -1, -1, NULL, 0);
    int best_audio = av_find_best_stream(fmt, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);

    if (best_video >= 0) {
        AVStream          *vs  = fmt->streams[best_video];
        AVCodecParameters *vp  = vs->codecpar;
        info->video_stream_idx = best_video;
        info->width            = vp->width;
        info->height           = vp->height;
        AVRational fr = vs->avg_frame_rate.num ? vs->avg_frame_rate : vs->r_frame_rate;
        info->fps = (fr.den > 0) ? (double)fr.num / fr.den : 0.0;
        const AVCodecDescriptor *d = avcodec_descriptor_get(vp->codec_id);
        ...
        if (sar.num > 0 && sar.den > 0 && sar.num != sar.den) {
            info->sar_num = sar.num;
            info->sar_den = sar.den;
        }
    }
...
int packet_queue_get(PacketQueue *q, AVPacket *out) {
    ...
    if (node->pkt) {
        ...
        av_packet_unref(out); /* pkt->data == NULL after this */
    }
    av_free(node);
    /* 0 = seek flush (caller flushes codec, continues loop)
       1 = EOS or real packet                                */
    return is_flush ? 0 : 1;
}
...
static int demux_thread(void *userdata) {
    ...
    while (!d->abort) {
        ...
        if (d->seek_pending) {
            ...
            /* Flush queues, then clear the interrupt flag so the sentinels
               can be enqueued, then send sentinels to wake decode threads */
            packet_queue_flush(&d->audio_queue);
            packet_queue_flush(&d->video_queue);
            SDL_LockMutex(d->audio_queue.mutex);
            d->audio_queue.interrupt = 0;
            SDL_UnlockMutex(d->audio_queue.mutex);
            SDL_LockMutex(d->video_queue.mutex);
            d->video_queue.interrupt = 0;
            SDL_UnlockMutex(d->video_queue.mutex);
            packet_queue_put_flush(&d->audio_queue);
            packet_queue_put_flush(&d->video_queue);
            d->seek_pending = 0;
        ...
        if (ret < 0) {
            ...
            packet_queue_put(&d->video_queue, NULL);
            break;
        }

        if (pkt->stream_index == d->audio_stream_idx)
            packet_queue_put(&d->audio_queue, pkt);
        else if (pkt->stream_index == d->video_stream_idx)
            packet_queue_put(&d->video_queue, pkt);
...
int demux_open(DemuxCtx *d, const char *path, char *errbuf, int errbuf_sz) {
    ...
    d->fmt_ctx->probesize           = 1024 * 1024;  /* 1 MB — default 5 MB too slow on SD */
    d->fmt_ctx->max_analyze_duration = 500000;       /* 0.5 s (µs) */
    ...
    d->audio_stream_idx = av_find_best_stream(d->fmt_ctx, AVMEDIA_TYPE_AUDIO,
                                               -1, -1, NULL, 0);
    d->video_stream_idx = av_find_best_stream(d->fmt_ctx, AVMEDIA_TYPE_VIDEO,
                                               -1, -1, NULL, 0);
    ...
    packet_queue_init(&d->video_queue);
...
int demux_cycle_audio(DemuxCtx *d) {
    ...
    /* Find current position in the list and advance */
    int pos = 0;
    ...
    /* Drop any queued packets from the old stream */
    packet_queue_flush(&d->audio_queue);
...
void demux_request_seek(DemuxCtx *d, double target_sec) {
    ...
    d->seek_pending = 1;
    SDL_UnlockMutex(d->seek_mutex);
    /* Wake the demux thread if it is blocked in pq_enqueue waiting for queue
       space.  Without this, the demux can't reach the seek check at the top
       of its loop until both packet queues drain naturally — potentially
       several seconds.  Setting interrupt=1 makes pq_enqueue return -1
       immediately (dropping the current packet), so the demux loops back and
       processes the seek within one av_read_frame call. */
    SDL_LockMutex(d->audio_queue.mutex);
    d->audio_queue.interrupt = 1;
    SDL_CondBroadcast(d->audio_queue.not_full);
    SDL_UnlockMutex(d->audio_queue.mutex);
    SDL_LockMutex(d->video_queue.mutex);
    d->video_queue.interrupt = 1;
    SDL_CondBroadcast(d->video_queue.not_full);
    SDL_UnlockMutex(d->video_queue.mutex);
}
