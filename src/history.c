...
void history_load(HistoryState *h) {
    ...
    h->recently_watched_count = resume_load_completed(&h->recently_watched);
}

void history_free(HistoryState *h) {
    free(h->in_progress);
    free(h->recently_watched);
    memset(h, 0, sizeof(*h));
}
    ...
    if (total == 0) {
        ...
        if (h->in_progress_count > 0) {
            ...
            for (int i = 0; i < h->in_progress_count; i++) {
                ...
                format_duration(e->position, pos_s, sizeof(pos_s));

                if (e->duration > 0.0) {
                    format_duration(e->duration, dur_s, sizeof(dur_s));
                    int pct = (int)(e->position / e->duration * 100.0);
                    if (pct > 100) pct = 100;
                ...
                /* Filled portion */
                if (e->duration > 0.0) {
                    double pct = e->position / e->duration;
                    if (pct > 1.0) pct = 1.0;
        ...
        if (h->recently_watched_count > 0) {
            ...
            fill_rect(renderer, 0, y, win_w, HEADER_H,
                      ...
                      dim(t->background.g, 10),
                      dim(t->background.b, 10), 0xff);
            draw_text(renderer, font_small, "RECENTLY WATCHED",
                      PADDING, y + (HEADER_H - TTF_FontHeight(font_small)) / 2,
                      win_w - PADDING * 2,
                      t->secondary.r, t->secondary.g, t->secondary.b);
            y += HEADER_H;
            ...
            for (int i = 0; i < h->recently_watched_count; i++) {
                ...
                int sel     = (nav_idx == h->selected);
                const char *path = h->recently_watched[i];
