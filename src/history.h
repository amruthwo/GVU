...
typedef struct {
    ...
    /* Recently-completed paths from history.dat (most-recent first) */
    char        (*recently_watched)[1024];
    int           recently_watched_count;
...
static inline int history_total(const HistoryState *h) {
    return h->in_progress_count + h->recently_watched_count;
}
