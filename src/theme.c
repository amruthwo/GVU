...
const Theme THEMES[] = {
    ...
    { "SPRUCE",
      ...
      /* icon: ring=tab, circle=body, outer=screen, center=tab */
      {0x32,0x64,0x46}, {0xd2,0xe1,0xd2}, {0x64,0x82,0x6e}, {0x32,0x64,0x46},
    ...
    { "monochrome",
      ...
      /* cover: body=secondary, tab=white, shadow=dark, screen=text, play=secondary */
      {0x80,0x80,0x80}, {0xf0,0xf0,0xf0}, {0x17,0x29,0x2d}, {0xf0,0xf0,0xf0}, {0x80,0x80,0x80},
      /* icon: ring=secondary, circle=black, outer=secondary, center=white */
      {0x80,0x80,0x80}, {0x00,0x00,0x00}, {0x80,0x80,0x80}, {0xf0,0xf0,0xf0},
      /* folder: tab=dark grey (distinct from mid-grey body), screen=white, body=secondary */
    ...
    { "light_contrast",
      ...
      /* icon: ring=body, circle=screen, outer=tab, center=body */
      {0xa3,0x51,0xc8}, {0xfa,0xfa,0xfa}, {0xa0,0xa0,0xa0}, {0xa3,0x51,0xc8},
      /* folder: tab, screen, body */
    ...
    { "light_sepia",
      ...
      /* icon: ring=screen, circle=body, outer=tab, center=screen */
      {0xfa,0xf0,0xdc}, {0x78,0x9c,0x70}, {0xa0,0xa0,0xa0}, {0xfa,0xf0,0xdc},
      /* folder: tab, screen=custom warm tan (background too light), body */
    ...
    { "vampire",
      ...
      /* icon: ring=body, circle=shadow, outer=body, center=tab */
      {0x60,0x00,0x00}, {0x00,0x00,0x00}, {0x60,0x00,0x00}, {0xc0,0x00,0x00},
      /* folder: tab=dark charcoal, screen=red, body=dark red */
    ...
    { "coffee_dark",
      ...
      /* icon: ring=tab, circle=tab, outer=body, center=screen */
      {0x6f,0x4e,0x37}, {0x6f,0x4e,0x37}, {0xf5,0xe6,0xd3}, {0xa0,0x8c,0x78},
      /* folder: tab, screen, body */
    ...
    { "cream_latte",
      ...
      /* icon: ring=tab, circle=body, outer=shadow, center=tab */
      {0x78,0x64,0x50}, {0xd2,0xb4,0x8c}, {0x2b,0x1f,0x16}, {0x78,0x64,0x50},
      /* folder: tab, screen, body */
    ...
    { "nautical",
      ...
      /* icon: ring=screen, circle=tab, outer=body, center=screen */
      {0xff,0xdc,0x64}, {0x38,0x58,0x9a}, {0x78,0x8c,0xb4}, {0xff,0xdc,0x64},
      /* folder: tab, screen, body */
    ...
    { "nordic_frost",
      ...
      /* icon: ring=body, circle=tab, outer=screen, center=shadow */
      {0x2e,0x34,0x40}, {0x88,0xc0,0xd0}, {0x81,0xa1,0xc1}, {0x17,0x29,0x2d},
      /* folder: tab, screen, body */
    ...
    { "night",
      ...
      /* icon: ring=tab, circle=shadow, outer=body, center=tab */
      {0x7a,0xb2,0xde}, {0x0d,0x0d,0x10}, {0x52,0x52,0x5e}, {0x7a,0xb2,0xde},
      /* folder: tab, screen, body=secondary (background too dark) */
...
static void load_key_file(const char *path, char *dst, size_t dst_sz) {
    if (dst[0]) return;  /* already set — gvu.conf takes priority */
    FILE *f = fopen(path, "r");
    if (!f) return;
    char line[256];
    while (fgets(line, sizeof(line), f)) {
...
void config_load_api_keys(const char *api_dir) {
    char path[768];
    snprintf(path, sizeof(path), "%s/SubDL_API.txt", api_dir);
    load_key_file(path, g_subdl_key, sizeof(g_subdl_key));
    snprintf(path, sizeof(path), "%s/TMDB_API.txt", api_dir);
    load_key_file(path, g_tmdb_key, sizeof(g_tmdb_key));
}
...
/* Original fill colors in default_folder.svg (same palette as default_cover.svg).
   Replace with per-theme folder_* values. */
static const char *FOLDER_ORIG_TAB    = "#71c6c4";  /* "Tab" path   */
static const char *FOLDER_ORIG_SCREEN = "#dde5e8";  /* "Screen" path */
static const char *FOLDER_ORIG_BODY   = "#afc3c9";  /* "Body" path  */
...
/* -------------------------------------------------------------------------
 * App icon SVG recoloring  (app_icon.svg — separate from default_cover)
 * ---------------------------------------------------------------------- */

/* Original fill colors in app_icon.svg, in replacement order. */
static const char *ICON_ORIG_RING   = "#666666";  /* outer ring outline     */
static const char *ICON_ORIG_CIRCLE = "#9dac93";  /* center circle fill     */
static const char *ICON_ORIG_OUTER  = "#cccccc";  /* left + right arrows    */
static const char *ICON_ORIG_CENTER = "#5d6c53";  /* center arrow           */

static char *svg_recolor_icon(const char *svg_orig, const Theme *theme) {
    size_t len = strlen(svg_orig);
    size_t buf_size = len + 256;
    char *buf = malloc(buf_size);
    if (!buf) return NULL;
    ...
    char hex[8];
    rgb_to_hex(&theme->icon_ring,   hex); str_replace_inplace(buf, buf_size, ICON_ORIG_RING,   hex);
    rgb_to_hex(&theme->icon_circle, hex); str_replace_inplace(buf, buf_size, ICON_ORIG_CIRCLE, hex);
    rgb_to_hex(&theme->icon_outer,  hex); str_replace_inplace(buf, buf_size, ICON_ORIG_OUTER,  hex);
    rgb_to_hex(&theme->icon_center, hex); str_replace_inplace(buf, buf_size, ICON_ORIG_CENTER, hex);

    return buf;
}

static unsigned char *svg_rasterize_icon(const char *svg_path) {
    char *svg_orig = read_file(svg_path);
    if (!svg_orig) {
        fprintf(stderr, "theme: could not read %s\n", svg_path);
        return NULL;
    ...
    char *svg_colored = svg_recolor_icon(svg_orig, theme_get());
    free(svg_orig);
    if (!svg_colored) return NULL;
    ...
    free(svg_colored);
    if (!img) {
        fprintf(stderr, "theme: nsvgParse failed for app icon\n");
        return NULL;
    }
...
int theme_save_app_icon(const char *svg_path, const char *png_path) {
    unsigned char *pixels = svg_rasterize_icon(svg_path);
    if (!pixels) return -1;
    ...
    free(pixels);
    if (ret != 0)
        fprintf(stderr, "theme_save_app_icon: %s\n", IMG_GetError());
    return ret == 0 ? 0 : -1;
}
